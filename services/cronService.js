...
function startCronJobs() {
  ...
  cron.schedule('0 2 * * *', async () => {
    const localToday = getCurrentDateInTimezone();
    const today = localToday.getDate();
    const todayIso = `${localToday.getFullYear()}-${String(localToday.getMonth() + 1).padStart(2, '0')}-${String(today).padStart(2, '0')}`;
    ...
    for (const c of customers) {
      ...
      const temporaryActiveUntil = String(c.temporary_active_until || '').trim();

      if (temporaryActiveUntil && temporaryActiveUntil >= todayIso) {
        logger.info(`[CRON] Melewati isolir ${c.name}: aktif sementara sampai ${temporaryActiveUntil}.`);
        continue;
      }

      if (temporaryActiveUntil && temporaryActiveUntil < todayIso) {
        customerSvc.clearTemporaryActivation(c.id);
      }
  ...
  cron.schedule('0 9 * * *', async () => {
    ...
    const defaultTemporaryTemplate =
      `Yth. Pelanggan {{nama}},\n\n` +
      `Akses internet Anda sedang dibuka sementara sampai *{{tanggal_aktif}}*.\n\n` +
      `Mohon segera melakukan pembayaran tagihan sebesar *Rp {{tagihan}}* agar layanan tetap aktif dan tidak terisolir kembali.\n\n` +
      `Silakan melakukan pembayaran melalui portal pelanggan: {{link}}\n\n` +
      `Terima kasih.\nSalam,\nAdmin ${getSetting('company_header', 'ISP')}`;
    const temporaryTemplate = String(db.getAppSetting('whatsapp_temporary_active_message', defaultTemporaryTemplate) || defaultTemporaryTemplate);
    const temporaryReminderDays = Math.max(0, Math.min(31, Number(getSetting('whatsapp_temporary_active_reminder_days', 1)) || 0));
    const temporaryReminderDate = addDaysToIsoDate(
      `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`,
      temporaryReminderDays
    );
    ...
    for (const c of customers) {
      ...
      const daysBeforeIsolir = Math.max(0, Math.min(31, Number(getSetting('whatsapp_auto_billing_days_before_isolir', 1)) || 0));
      const reminderDay = dueDay - daysBeforeIsolir;
      const temporaryReminder = String(c.temporary_active_until || '').trim() === temporaryReminderDate;
      const shouldSend = (reminderDay >= 1 && day === reminderDay) || temporaryReminder;
      if (!shouldSend) continue;

      seenPhones.add(digits);
      c._temporaryReminder = temporaryReminder;
    ...
    for (let i = 0; i < targetCustomers.length; i++) {
      ...
      while (attemptCount < maxAttempts) {
        try {
          ...
          const messageTemplate = c._temporaryReminder ? temporaryTemplate : template;
          let formattedMsg = messageTemplate
            ...
            .replace(/{{tanggal_aktif}}/gi, c.temporary_active_until || '-')
