...
async function activateCustomerTemporarily(id, untilDate) {
  const normalizedUntilDate = String(untilDate || '').trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(normalizedUntilDate)) {
    throw new Error('Tanggal aktif sementara tidak valid');
  }
  const parsedDate = new Date(`${normalizedUntilDate}T00:00:00Z`);
  if (Number.isNaN(parsedDate.getTime()) || parsedDate.toISOString().slice(0, 10) !== normalizedUntilDate) {
    throw new Error('Tanggal aktif sementara tidak valid');
  }
  ...
  if (normalizedUntilDate < todayIso) {
    throw new Error('Tanggal aktif sementara tidak boleh sudah lewat');
  }

  await activateCustomer(id);
  db.prepare('UPDATE customers SET temporary_active_until = ? WHERE id = ?').run(normalizedUntilDate, id);
  return true;
}

function clearTemporaryActivation(id) {
