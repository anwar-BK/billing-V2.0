...
router.post('/customers', requireTechSession, express.urlencoded({ extended: true }), async (req, res) => {
  try {
    ...
    const customerData = {
      ...
      status: String(req.body.billing_type || 'postpaid').trim() === 'prepaid' ? 'suspended' : (String(req.body.status || 'active').trim() || 'active'),
      billing_type: String(req.body.billing_type || 'postpaid').trim() === 'prepaid' ? 'prepaid' : 'postpaid',
