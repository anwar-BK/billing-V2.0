...
router.post('/customers/:id/temporary-activate', requireAdminSession, async (req, res) => {
  try {
    const untilDate = String(req.body.until_date || '').trim();
    await customerSvc.activateCustomerTemporarily(req.params.id, untilDate);
    const customer = customerSvc.getCustomerById(req.params.id);
    req.session._msg = { type: 'success', text: `Pelanggan "${customer.name}" aktif sementara sampai ${untilDate}.` };
  } catch (e) {
    req.session._msg = { type: 'error', text: 'Gagal mengaktifkan sementara: ' + e.message };
  }
  res.redirect('back');
});
...
router.post('/settings', requireAdminSession, express.urlencoded({ extended: true }), (req, res) => {
  try {
    ...
    if (success) {
      ...
      import('../services/whatsappBot.mjs').then((whatsappBot) => {
        if (newSettings.whatsapp_enabled) {
          return whatsappBot.startWhatsAppBot();
        }
        return null;
      }).catch(e => logger.error('Gagal menyinkronkan WhatsApp Bot setelah menyimpan pengaturan:', e.message));
...
router.post('/whatsapp/auto-billing', requireAdminSession, express.urlencoded({ extended: true }), (req, res) => {
  try {
    ...
    const daysBeforeIsolir = req.body && req.body.days_before_isolir !== undefined
      ? parseInt(req.body.days_before_isolir)
      : 1;
    ...
    const next = {
      ...
      whatsapp_auto_billing_days_before_isolir: Number.isFinite(daysBeforeIsolir)
        ? Math.max(0, Math.min(31, daysBeforeIsolir))
        : 1,
