...
/**
 * Docker sering mem-mount file yang belum ada sebagai FOLDER.
 * SQLite lalu crash ("unable to open database file") saat install pertama.
 */
function resolveSqlitePath(preferredPath) {
  try {
    if (fs.existsSync(preferredPath) && fs.statSync(preferredPath).isDirectory()) {
      const inner = path.join(preferredPath, 'billing.db');
      console.warn('[DB] ' + preferredPath + ' adalah folder, bukan file. Database disimpan di: ' + inner);
      return inner;
    }
  } catch (e) {
    console.warn('[DB] Gagal memeriksa path database:', e.message);
  }
  return preferredPath;
}

const dbPath = resolveSqlitePath(path.join(dbDir, 'billing.db'));
...
try {
  ...
  console.error('[DB] Pastikan folder database bisa ditulis, dan billing.db bukan folder (salah mount Docker).');
  process.exit(1);
}

function runSchema(sql, label) {
  try {
    db.exec(sql);
  } catch (err) {
    console.error('[DB] Gagal inisialisasi skema (' + label + '):', err.message);
    throw err;
  }
}
...
try {
  db.exec("ALTER TABLE customers ADD COLUMN temporary_active_until DATE");
} catch (e) { /* ignore if already exists */ }
try {
